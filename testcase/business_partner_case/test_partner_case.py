...
class TestPartnerPage:
    ...
    @pytest.mark.P2
    @allure.testcase(url="", name='新增业务伙伴的新增2个联系人')
    def test_create_new_bro_verification(self):

        assert_info = self.partner.create_new_bro_verification()
    ...
    @pytest.mark.P2
    @allure.testcase(url="", name='创建业务伙伴账号必填校验')
    def test_create_account_required(self):

        assert_info = self.partner.create_account_required()
        self.partner.assert_allure_screenshot(assert_info, "请填写该必填项")

    @pytest.mark.P2
    @allure.testcase(url="", name='创建业务伙伴账号长度校验')
    def test_create_account_length(self):

        assert_info = self.partner.create_account_length()
        self.partner.assert_allure_screenshot(assert_info[0], "6~18位字符，只可使用英文字母、数字、下划线、”@”和“.”")
        self.partner.assert_allure_screenshot(assert_info[1], "请输入18个字以内的内容")

    @pytest.mark.P2
    @allure.testcase(url="", name='账号的汉族，特殊符号格式校验')
    def test_create_account_chinese(self):

        assert_info = self.partner.create_account_chinese()
        self.partner.assert_allure_screenshot(assert_info[0], "6~18位字符，只可使用英文字母、数字、下划线、”@”和“.”")
        self.partner.assert_allure_screenshot(assert_info[1], "6~18位字符，只可使用英文字母、数字、下划线、”@”和“.”")

    @pytest.mark.P2
    @allure.testcase(url="", name='账号密码，随机生成')
    def test_create_account_random_generation(self):

        assert_info = self.partner.create_account_random_generation()
        self.partner.assert_allure_screenshot(assert_info, True)

    @pytest.mark.P2
    @allure.testcase(url="", name='账号的手机号格式校验')
    def test_create_phone_verification(self):

        assert_info = self.partner.create_phone_verification()
        self.partner.assert_allure_screenshot(assert_info, "请输入正确的手机号码")

    @pytest.mark.P2
    @allure.testcase(url="", name='账号的邮箱格式校验')
    def test_create_email_verification(self):

        assert_info = self.partner.create_email_verification()
        self.partner.assert_allure_screenshot(assert_info, "请输入正确的邮箱")
